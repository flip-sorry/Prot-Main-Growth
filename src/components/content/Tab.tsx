interface TabProps {
  label: string;
  count: number;
  active?: boolean;
  onClick?: () => void;
  className?: string;
}

export default function Tab({ label, count, active = false, onClick, className = '' }: TabProps) {
  return (
    <div
      className={`flex flex-col items-start min-w-[100px] md:min-w-[120px] shrink-0 cursor-pointer ${className}`}
      onClick={onClick}
    >
      <div
        className={`flex flex-col gap-1 isolate items-start leading-[1.2] overflow-hidden pl-2 md:pl-3 pr-3 md:pr-5 py-2 md:py-3 shrink-0 text-xs md:text-sm w-full transition-colors duration-150 ${
          active 
            ? 'bg-background rounded-tl-lg rounded-tr-lg' 
            : 'bg-[rgba(255,255,255,0.08)] hover:bg-[rgba(118,118,118,0.08)] rounded-tl-lg rounded-tr-lg'
        }`}
      >
        <p className={`font-semibold relative shrink-0 z-10 text-text-dark`} style={{ fontFamily: "'Graphik LC Web', sans-serif" }}>
          {label}
        </p>
        <p className="font-normal relative shrink-0 text-text-light z-0" style={{ fontFamily: "'Graphik LC Web', sans-serif" }}>
          {count} docs
        </p>
      </div>
    </div>
  );
}

